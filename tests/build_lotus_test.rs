...
/// Test that the Lotus build command works end-to-end
#[test]
fn test_lotus_build_integration() {
    // Skip this test if Docker is not available
    if !docker_available() {
        println!("Skipping Lotus build test: Docker not available");
        return;
    }
    ...
    assert!(help_output.status.success(), "Build lotus help command failed");
    let help_text = String::from_utf8_lossy(&help_output.stdout);
    assert!(help_text.contains("Lotus"), "Help text doesn't mention Lotus");
