...
pub fn check_requirements(setup: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cfg!(target_os = "macos") {
        if !is_command_available("brew") {
            if setup {
                println!("{}", "❌ Homebrew not found. Attempting to install Homebrew...".yellow());
                setup_docker::install_homebrew()?;
                ...
                eprintln!("{}", "❌ Error: Homebrew is not installed or not available in PATH.".red().bold());
                eprintln!("{}", "Homebrew is required for automatic Docker installation on macOS.".cyan());
                eprintln!("{}", "Please install Homebrew from https://brew.sh/".cyan());
                eprintln!("{}", "Or run with --setup flag to attempt automatic installation.".cyan());
                return Err("Homebrew not available".into());
            }
        } else {
    ...
    if !is_command_available("docker") {
        if setup {
            println!("{}", "❌ Docker not found. Attempting to install Docker...".yellow());
            setup_docker::install_docker()?;
        } else {
            eprintln!("{}", "❌ Error: Docker is not installed or not available in PATH.".red().bold());
            eprintln!("{}", "Please install Docker Desktop from https://www.docker.com/products/docker-desktop".cyan());
            eprintln!("{}", "Or run with --setup flag to attempt automatic installation.".cyan());
